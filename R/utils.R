...
#' @keywords internal
validate_and_parse_df <- function(df, count_col = NULL) {
  if (!is.data.frame(df)) {
  ...
  # Hierarchical columns are all columns except the count column
  hier_indices <- setdiff(seq_len(ncol(df)), count_index)
